...
=begin
  Some classes (for example the standard library's socket class) define 
  their own send method which has nothing to do with Object#send. So if 
  you want to work with objects of any class, you need to use __send__ to
   be on the safe side. __send__ was implemented later than send, which
   was originally implemented, hence the underscores.
=end
  end
